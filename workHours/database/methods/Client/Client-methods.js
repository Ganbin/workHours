...
//debugger;
	...
	workTimes.forEach(function (workTime){
		...
		// Calculate the total
		subTotal.time += (((workTime.end - workTime.start) - workTime.breakTimeMs)/1000/60); // Compute the time for each category in minute.
		subTotal.price += (((((workTime.end - workTime.start) + (workTime.trainTimeMS)) - workTime.breakTimeMs)/1000/60/60)*workTime.categoryPriceHour); // Compute the price for each category.
		subTotal.addPrice += workTime.trainPrice; // Compute the train price for each category .
		subTotal.trainTime += workTime.trainTimeMS/1000/60; // Compute the train time for each category in minute.
	});
